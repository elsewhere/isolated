...
namespace democore
{
	...
	FMOD::System *Sound::fmodSystem = nullptr;
	...
	bool Song::loadStream(const std::string& name)
	{
		FMOD_RESULT result;
		FILE *fp;
		...
		if (!fp)
		{			
			g_debug << "Cannot load sound stream " << name << std::endl;
			return false;
		}
		...
		if( result != FMOD_OK )
		{
			g_debug << "Sound::init() error from FMOD! error = " << FMOD_ErrorString(result) << std::endl;
			return false;
		}
