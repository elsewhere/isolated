...
VERTEX_SHADER
{
	...
	void main() 
	{
		...
		if (particleEnergyOut <= 0.0)
		{
		   ...
		   vec3 generatedPos = vec3((u - 0.5), (v - 0.5), 0.0) * 5;
//		   vec3 generatedPos = vec3(sin(a) * sin(b), sin(a) * cos(b), cos(a));
		   ...
		   vec4 c1 = vec4(0.9, 0.5, 0.2, 1.0);
		   vec4 c2 = vec4(1.0, 0.8, 0.5, 1.0);

		   vec4 color = mix(c1, c2, randhash(seed++, 1.0));

		   particleColorOut = texture2D(tex, vec2(u, 1.0 - v)) * color;
		}
		else
		{
