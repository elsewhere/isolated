...
VERTEX_SHADER
{
	...
	void main() 
	{
		particleDirectionOut = getDirection(particlePosition);
		particlePositionOut = particlePosition + particleDirectionOut * 0.01;
		...
		if (particleEnergyOut <= 0.0)
		{
		   ...
		   particleDirectionOut = vec3(sin(c) * sin(d), sin(c) * cos(d), cos(c)) * 1.0;
